...
| DCR-RUNE-007 | Compatibility negotiation | implemented retained report slice | Retained `check-compatibility` validates source/target refs, versions, unsupported concepts, degraded behavior, and runtime-host blocking. No automatic migration, best-effort conversion, live inspection, mutation, runtime host behavior, or policy enforcement is approved. |
